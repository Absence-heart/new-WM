          ...
          <li v-for="(items,index) in goods" :key="index" class="foods-list-hook">
            ...
            <ul>
              <li v-for="(item2,index) in items.foods" :key="index" class="items-content-wrapper">
                <div class="content" @click="getDetail(item2)">
                  ...
                  <div class="items-content border-1px">
                    ...
                    <div class="prices">
                      <span class="yuan">￥</span><span class="price">{{item2.price}}</span><span v-if="item2.oldPrice" class="oldPrice" style="margin-left:8px"><span class="yuan">￥</span>{{item2.oldPrice}}</span>
                    </div>
                  </div>
...
export default {
  ...
  created () {
    ...
    axios.get('/good/goods').then(res => {
      ...
      Vue.nextTick(() => { /** 在下次DOM更新循环结束之后执行延迟回调。在数据修改后立即使用这个办法，获取更新之后的DOM */
        that._initscroll()
